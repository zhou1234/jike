...
public class Passenger implements Serializable {
	...
	public Passenger(String jsonString, String systype) {// "systype":"0国内 1国际 2火车票"
															// 国际证件号码默认取护照，国内、火车默认取身份证
		try {
			JSONObject json = new JSONObject(jsonString);
			if (jsonString.contains("passengerType"))
				this.passengerType = json.getString("passengerType");
			if (jsonString.contains("passengerName"))
				this.passengerName = json.getString("passengerName");
			else
				this.passengerName = json.getString("CName");
			if (jsonString.contains("identificationType"))
				this.identificationType = json.getString("identificationType");
			if (jsonString.contains("identificationNum"))
				this.identificationNum = json.getString("identificationNum");
			// if(jsonString.contains("mobile"))this.mobile=json.getString("mobile");
			if (jsonString.contains("addto"))
				this.addto = json.getString("addto");
			...
			if (this.passengerName == null || this.passengerName.equals("")) {
				...
				if (this.CName.equals("") || this.CName.equals(null)
						|| this.CName.equals("null")) {
					this.passengerName = this.EName;
				}
			}
			if (systype.equals("1")) {// 国际机票 取英文名
				this.passengerName = this.EName;
				if (jsonString.contains("Sex"))
					this.gender = json.getString("Sex");
				if (jsonString.contains("Country"))
					this.nation = json.getString("Country");
				if (jsonString.contains("CusBirth"))
					this.birthDay = json.getString("CusBirth");
				if (jsonString.contains("Qianfadi"))
					this.issueAt = json.getString("Qianfadi");
			}
			int idtype = 0;// 证件类型
			...
			if ((systype.equals("1"))) {// 国际机票 根据证件类型取有效期 IDdeadline;//证件有效期
				...
				switch (idtype) {
				case 1:
					if (jsonString.contains("Hzdate"))
						d = json.getString("Hzdate");
					try {
					...
					break;
				case 5:
					if (jsonString.contains("Tbdate"))
						d = json.getString("Tbdate");
					try {
						this.IDdeadline = DateUtil.getDate(d);
					} catch (ParseException e) {
						e.printStackTrace();
					...
					break;
				case 4:
					if (jsonString.contains("Ggdate"))
						d = json.getString("Ggdate");
					try {
						this.IDdeadline = DateUtil.getDate(d);
					} catch (ParseException e) {
						e.printStackTrace();
